...
def run_ORA(top_genes, number_of_genes_in_deg, genes_by_pathway):
    ...
    differenttially_expressed_genes_by_pathway = get_number_of_differenttially_expressed_genes_by_pathway(
...
def get_number_of_differenttially_expressed_genes_by_pathway(top_genes, gmt_file):
    """
    key = pathway
    value =  list of genes members of that pathway and reported as differentially expressed

    """
    number_of_differenttially_expressed_genes_by_pathway = {
        pathway: len(set(genes) & set(top_genes)) for pathway, genes in gmt_file.items()
    }
    return number_of_differenttially_expressed_genes_by_pathway
...
def run_hipergeometric_tests(
    ...
    result = {
        pathway: stats.hypergeom.sf(
            number_of_differenttially_expressed_genes_by_pathway[pathway] - 1,
            number_of_genes_in_deg,
